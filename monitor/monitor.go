...
func AddMonitorFile(anode map[string]string) (err error) {
	...
	if err != nil {
		logs.Error("AddMonitorFile error getting files from database: %s", err.Error())
		return err
	}
	for x := range files {
	...
	if err != nil {
		logs.Error("AddMonitorFile error inserting path into database: %s", err.Error())
		return err
	}
	err = ndb.InsertMonitorValue(uuid, "maxSize", anode["maxSize"])
	if err != nil {
		logs.Error("AddMonitorFile error inserting maxSize into database: %s", err.Error())
		return err
	}
	err = ndb.InsertMonitorValue(uuid, "maxDays", anode["maxDays"])
	if err != nil {
		logs.Error("AddMonitorFile error inserting maxDays into database: %s", err.Error())
		return err
	}
	err = ndb.InsertMonitorValue(uuid, "rotate", anode["rotate"])
	if err != nil {
		logs.Error("AddMonitorFile error inserting rotate into database: %s", err.Error())
		return err
	}
	err = ndb.InsertMonitorValue(uuid, "maxLines", anode["maxLines"])
	if err != nil {
		logs.Error("AddMonitorFile error inserting maxLines into database: %s", err.Error())
		return err
	}
...
func DeleteMonitorFile(anode map[string]string) (err error) {
	...
	if err != nil {
		logs.Error("DeleteMonitorFile error inserting new path into database: %s", err.Error())
		return err
	}
...
func PingMonitorFiles() (data map[string]map[string]string, err error) {
	...
	if err != nil {
		logs.Error("PingMonitorFiles error getting monitor paths: %s", err.Error())
		return nil, err
	}
...
func ChangeRotationStatus(anode map[string]string) (err error) {
	...
	if err != nil {
		logs.Error("ChangeRotationStatus error updating monitor file value: %s", err.Error())
		return err
	}
