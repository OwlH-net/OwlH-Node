...
func GetIncidentsNode() (anode map[string]map[string]string, err error) {
	...
	if err != nil {
		logs.Error("GetIncidentsNode error getting incidents from database %s", err.Error())
		return nil, err
	}
...
func PutIncidentNode(anode map[string]string) (err error) {
	...
	if err != nil {
		logs.Error("PutIncidentNode error putting incidents into database: %s", err.Error())
		return err
	}
	return nil
}
