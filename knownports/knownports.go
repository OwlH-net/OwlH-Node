...
import (
	...
	// "github.com/OwlH-net/OwlH-Node/analyzer"
	"encoding/json"
	"net"
	"os"
	"regexp"
...
func NewPorts() {
	...
	for Status != "Disabled" {
		if Status == "Reload" {
			...
			if err != nil {
				logs.Error("Error changing status from Realod status: " + err.Error())
			}
		}
		...
		if err != nil {
			logs.Error("Error LoadPortsData: " + err.Error())
		}
...
func LoadPortsData() (data map[string]map[string]string, err error) {
	...
	if err != nil {
		logs.Error("knownports LoadPortsData -- Can't read query result: %s", err.Error())
		return nil, err
	}
	return values, nil
}
