...
import (
	...
	"github.com/OwlH-net/OwlH-Node/zeek"
...
func LoadNetworkValuesSelected() (values map[string]map[string]string, err error) {
	...
	if err != nil {
		logs.Error("LoadNetworkValuesSelected Error reading nodeconfig values for Node: " + err.Error())
		return nil, err
	}
	return values, err
}
...
func UpdateNetworkInterface(data map[string]string) (err error) {
	...
	err = ndb.UpdatePluginValue(data["service"], "interface", data["value"])
	if err != nil {
		logs.Error("UpdateNetworkInterface Zeek interface update Error: " + err.Error())
		return err
	}
	err = zeek.SyncCluster(data, "standalone")
	if err != nil {
		logs.Error("UpdateNetworkInterface Zeek update interface and node.cfg Error: " + err.Error())
		return err
	}
	// //restart zeek
	// err = zeek.DeployZeek()
