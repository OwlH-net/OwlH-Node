...
func ChangeDataflowValues(anode map[string]string) (err error) {
	...
	if err != nil {
		logs.Error("ChangeDataflowValues UPDATE error for update dataflow values: " + err.Error())
		return err
	}
	return nil
}
...
func LoadDataflowValues() (data map[string]map[string]string, err error) {
	...
	if err != nil {
		logs.Error("LoadDataflowValues error loading dataflow values: " + err.Error())
		return nil, err
	}
	return data, nil
}
...
func SaveSocketToNetwork(anode map[string]string) (err error) {
	...
	if exists {
		...
		if err != nil {
			logs.Error("SaveSocketToNetwork error inserting name dataflow values: " + err.Error())
			return err
		}
		err = ndb.InsertDataflowValues(uuid, "cert", anode["cert"])
		if err != nil {
			logs.Error("SaveSocketToNetwork error inserting cert dataflow values: " + err.Error())
			return err
		}
		err = ndb.InsertDataflowValues(uuid, "interface", anode["interface"])
		if err != nil {
			logs.Error("SaveSocketToNetwork error inserting interface dataflow values: " + err.Error())
			return err
		}
		err = ndb.InsertDataflowValues(uuid, "port", anode["port"])
		if err != nil {
			logs.Error("SaveSocketToNetwork error inserting port dataflow values: " + err.Error())
			return err
		}
		err = ndb.InsertDataflowValues(uuid, "type", "sockettonetwork")
		if err != nil {
			logs.Error("SaveSocketToNetwork error inserting sockettonetwork dataflow values: " + err.Error())
			return err
		}
	}
...
func DeleteDataFlowValueSelected(anode map[string]string) (err error) {
	...
	if err != nil {
		logs.Error("DeleteDataFlowValueSelected error deleting a socket->network: " + err.Error())
		return err
	}
	return nil
}
