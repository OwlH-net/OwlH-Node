...
func GetAllServers() (data map[string]map[string]string, err error) {
	...
	for rows.Next() {
		...
		if allservers[uniqueid] == nil {
			allservers[uniqueid] = map[string]string{}
		}
...
func UpdateStapByparam(param string, value string) (err error) {
	...
	if err != nil {
		logs.Error("UpdateStapByparam UPDATE prepare error: " + err.Error())
		return err
	}

	_, err = updateStapNode.Exec(&value, &param)
	if err != nil {
		logs.Error("UpdateStapByparam UPDATE exec error: " + err.Error())
		return err
	}
