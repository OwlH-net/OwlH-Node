...
func InsertSuricataGroupValue(uuid string, param string, value string) (err error) {
	if Groupdb == nil {
		logs.Error("no access to database group")
		return errors.New("no access to database group")
	}

	stmt, err := Groupdb.Prepare("insert into suricata (suri_uniqueid, suri_param, suri_value) values (?,?,?);")
	if err != nil {
		logs.Error("InsertSuricataGroupValue Prepare error: %s", err.Error())
		return err
	}

	_, err = stmt.Exec(&uuid, &param, &value)
	if err != nil {
		logs.Error("InsertSuricataGroupValue Execute error: %s", err.Error())
		return err
	}
...
func UpdateSuricataGroupValue(uuid string, param string, value string) (err error) {
	...
	if err != nil {
		logs.Error("updateSuricataGroupValue Prepare error: %s", err.Error())
		return err
	}
	...
	defer gr.Close()
	if err != nil {
		logs.Error("updateSuricataGroupValue Execute error: %s", err.Error())
		return err
	}
...
func GetAllGroupData() (data map[string]map[string]string, err error) {
	...
	if err != nil {
		logs.Error("GetAllGroupData Groupdb.Query Error : %s", err.Error())
		return nil, err
	}
	...
	for rows.Next() {
		if err = rows.Scan(&uniqid, &param, &value); err != nil {
			logs.Error("GetAllGroupData -- Query return error: %s", err.Error())
			return nil, err
		}
