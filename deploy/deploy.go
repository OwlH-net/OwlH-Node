...
func CheckDeployFiles()(anode map[string]string){
    ...
    suricata, err := utils.GetKeyValueString("deploy", "suricata"); if err != nil { logs.Error("SendFile Error getting suricata from main.conf"); return nil}
    zeek, err := utils.GetKeyValueString("deploy", "zeek"); if err != nil { logs.Error("SendFile Error getting zeek from main.conf"); return nil}
    moloch, err := utils.GetKeyValueString("deploy", "moloch"); if err != nil { logs.Error("SendFile Error getting moloch from main.conf"); return nil}
    iface, err := utils.GetKeyValueString("deploy", "interface"); if err != nil { logs.Error("SendFile Error getting interface from main.conf"); return nil}
    firewall, err := utils.GetKeyValueString("deploy", "firewall"); if err != nil { logs.Error("SendFile Error getting firewall from main.conf"); return nil}
